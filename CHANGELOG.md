...
- [OpenType] Initial support for OpenType features has landed! This requires a longer explanation, but in short:
  ...
  - Some glyph positioning is emulated on the fly using data in the font: `curs`, `kern`, `mark` and `mkmk`. This uses live kerning data and glyph anchors and allows the user to make edit and immediately see the effects in the rendered text.
